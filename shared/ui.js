window.appUI = {
  clearForm: function () {
    if (confirm("هل أنت متأكد من تفريغ جميع محتويات النموذج؟")) {
      ...
      const photoTexts = document.querySelectorAll(
        ".photo-box-text, .photo-box span",
      );
      photoTexts.forEach((el) => (el.style.display = "block"));

      if (window.currentPageKey) {
        window.appStorage.savePage(
          window.currentPageKey,
          window.getPageExtraData ? window.getPageExtraData() : {},
        );
      }

      // If there's a custom clear function on the page, call it
      if (typeof window.clearMembers === "function") {
        window.clearMembers(true);
      }
    }
  },
  ...
  savePdf: function () {
    ...
    if (titleEl) {
      title = titleEl.value
        ? titleEl.value.trim().replace(/\s+/g, "-")
        : titleEl.innerText.trim().replace(/\s+/g, "-");
    }

    // Trigger save first
    if (window.currentPageKey) {
      const extraData = window.getPageExtraData
    ...
    // Prepare inputs for html2canvas
